...
const pointAtSOnRoute = (
  state: { route: MapLatLng[]; segLens: number[]; cum: number[]; total: number },
  s: number,
): MapLatLng => {
...
export default function MapScreen({
  ...
  useEffect(() => {
    (async () => {
      ...
      const rawPolyline = r2.data?.polyline as { lat: number; lon: number }[] | undefined;
      if (rawPolyline?.length) {
        setRouteCoords(rawPolyline.map((p) => ({ latitude: p.lat, longitude: p.lon })));
      } else {
        setRouteCoords([]);
      }
    })();
  ...
  const depot = useMemo(() => {
    ...
    return first ? { latitude: first.lat, longitude: first.lon, nombre: first.nombre } : null;
  }, [wps]);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top','left','right']}>
      ...
      <MapView ref={mapRef} provider={PROVIDER_GOOGLE} style={styles.map} initialRegion={initialRegion}>
        ...
        {depot && (
          <Marker coordinate={depot} title={depot.nombre ?? 'Inicio de ruta'} anchor={{ x: 0.5, y: 0.5 }}>
            <View style={styles.depot}><Text style={{ fontSize: 16 }}>{ICONS.start}</Text></View>
          </Marker>
        )}
        {childHouse && (
          <Marker coordinate={childHouse} title={`${ICONS.child} Casa de tu hijo`} anchor={{ x: 0.5, y: 0.5 }}>
            <View style={styles.childMarker}><Text style={{ fontSize: 18 }}>{ICONS.child}</Text></View>
          </Marker>
        )}
        ...
        {hijos.filter((h) => h.id !== childId).map((h) => (
          <Marker
            ...
            anchor={{ x: 0.5, y: 0.5 }}
          >
            <View style={styles.otherHouse}><Text>{ICONS.house}</Text></View>
          </Marker>
        ))}

        {school && (
          <Marker coordinate={school} title="Colegio" anchor={{ x: 0.5, y: 0.5 }}>
            <View style={styles.school}><Text>{ICONS.school}</Text></View>
          </Marker>
        )}

        {busPos && (
          <Marker coordinate={busPos} title="Bus" flat rotation={heading} anchor={{ x: 0.5, y: 0.5 }}>
            <View style={styles.bus}><Text style={{ fontSize: 20 }}>{ICONS.bus}</Text></View>
          </Marker>
        )}
      </MapView>
      ...
      <View style={styles.controls}>
        ...
        <Pressable
          ...
          disabled={simulating}
        >
          <Text style={styles.btnTxt}>{simulating ? 'Simulando' : 'Iniciar'}</Text>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  depot: { width:32, height:32, borderRadius:16, backgroundColor:'#EAF2FF', alignItems:'center', justifyContent:'center', borderWidth:1, borderColor:'#3B82F6' },
